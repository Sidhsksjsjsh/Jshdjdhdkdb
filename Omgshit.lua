...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
	...
	local log = {
		...
		Blocked = blocked,
		Source = src,
		GenScript = "-- OMG THIS WAS SO LONG I COULD NOT LOAD MANY SCRIPTS🗣️😭",
	...
	for k, v in pairs(t) do -- iterates over table
		...
		if size > (_G.SimpleSpyMaxTableSize or 1000) then
			s = s
				.. "\n"
				.. string.rep(" ", l)
				.. "-- MAXIMUM TABLE SIZE REACHED, CHANGE '_G.RemoteSpyMaxTableSize' TO ADJUST MAXIMUM SIZE "
			break
		end
	...
	if keyToString then
		if typeof(v) == "userdata" and getrawmetatable(v) then
			return string.format(
				'"<void> (%s)" --[[Potentially hidden data (tostring in RemoteSpy:HookRemote/GetRemoteFiredSignal at your own risk)]]',
				safetostring(v)
			)
		elseif typeof(v) == "userdata" then
...
function formatstr(s, indentation)
	...
	return '"'
		...
		.. (
			reachedMax
				and " --[[ MAXIMUM STRING SIZE REACHED, CHANGE '_G.RemoteSpyMaxStringSize' TO ADJUST MAXIMUM SIZE ]]"
			or ""
		)
end
	...
	if not toggle then
		if hookmetamethod then
			...
			if not original then
				warn("RemoteSpy: namecall method not found!")
				onToggleButtonClick()
				return
			end
...
if not _G.SimpleSpyExecuted then
	...
	if not succeeded then
		warn(
			"A fatal error has occured, RemoteSpy was unable to launch properly.\nPlease DM this error message to C4#4172:\n\n"
				.. tostring(err)
		)
		SimpleSpy2:Destroy()
		hookfunction(remoteEvent.FireServer, originalEvent)
